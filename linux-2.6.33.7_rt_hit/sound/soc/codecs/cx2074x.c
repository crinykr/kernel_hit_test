...
static int cx2074x_register(struct cx2074x_priv * cx2074x)
{
	...
	if (cx2074x_codec) {
		dev_err(codec->dev, "Multiple CX20709 devices not supported\n");
		ret = -EINVAL;
		goto err;
	}
	...
	cx2074x_codec = codec;
	soc_codec_cx2074x_dai.dev = codec->dev;
	...
	ret = snd_soc_register_dais(&soc_codec_cx2074x_dai, 1);
	if (ret != 0) {
...
{
	snd_soc_unregister_codec(cx2074x_codec);
