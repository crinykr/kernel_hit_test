/*
* ALSA SoC CX2074X Channel codec driver
*
...
*      
*************************************************************************
*  Modified Date:  04/02/13
*  File Version:   2.6.33.4
*************************************************************************
*/
...
#define CX20865_FORMATS_CAPTURE ( SNDRV_PCM_FMTBIT_S16_LE \
...
struct snd_soc_codec *cx20865_codec;
...
static const char *cx20865_configurations[] = { "CONF", "ASR2", "ASR4", "VOI2", "VOI4", "DC16", "DC48", "MP16", "MPTS", "R6CH" };

static const struct soc_enum cx20865_enum[] = {
	SOC_ENUM_SINGLE(0, 0, 10, cx20865_configurations),
};
...
static int cx20865_config_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int cx20865_config_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = &cx2074x->codec1;
	struct soc_enum *e = (struct soc_enum *)kcontrol->private_value;
	Command cmd;
...
static const struct snd_kcontrol_new cx2074x_snd_controls[] = {
    ...
    //SOC_DOUBLE_R_EXT_TLV("Capture Volume", CX20865_MICROPHONE_L, CX20865_MICROPHONE_R, 0, 48, 0, cx20865_get_volsw, cx20865_put_volsw, adc_tlv_cx20865),

    //                 (xname, xreg, shift_left, shift_right, xmax, xinvert, xhandler_get, xhandler_put, tlv_array)
    //SOC_DOUBLE_EXT_TLV("Capture Volume", CX20865_MICROPHONE_L, 0, 5, 48, 0, cx20865_get_volsw, cx20865_put_volsw, adc_tlv_cx20865),
	
    //            (xname, xreg, shift_left, shift_right, xmax, xinvert, xhandler_get, xhandler_put)
    //SOC_DOUBLE_EXT("Capture Volume", CX20865_MICROPHONE_L, 0, 5, 48, 0, cx20865_get_volsw, cx20865_put_volsw),
    ...
    { .iface = SNDRV_CTL_ELEM_IFACE_MIXER,
      .name = "CX20865 Configuration Switch",
      .info = snd_soc_info_enum_double,
      .get = cx20865_config_get,
      .put = cx20865_config_put,
...
struct snd_soc_dai soc_codec_cx2074x_cx20865_dai =
{
    .name				= "CX2074X+CX20865",
    .ops				= &cx2074x_dai_ops,
	.symmetric_rates 	= 1,
    .capture = {
        .stream_name	="Capture",
		.rates			= CX20865_RATES_CAPTURE,
		.formats		= CX20865_FORMATS_CAPTURE,
		.channels_min	= 1,
		.channels_max	= 2,
    },
    .playback = {
...
EXPORT_SYMBOL_GPL(soc_codec_cx2074x_cx20865_dai);
...
struct snd_soc_codec_device soc_codec_dev_cx2074x_cx20865 = {
    .probe =cx2074x_probe,
    .remove = cx2074x_remove,
    .suspend = cx2074x_suspend,
    .resume = cx2074x_resume,
};
EXPORT_SYMBOL_GPL(soc_codec_dev_cx2074x_cx20865);
...
static int cx2074x_register(struct cx2074x_priv * cx2074x)
{
	...
	if (cx2074x_codec || cx20865_codec) {
		dev_err(codec->dev, "Multiple CX2074X/CX20865 devices not supported\n");
		ret = -EINVAL;
		goto err;
	}
	...
	codec->dai				= &soc_codec_cx2074x_cx20865_dai;
	codec->num_dai			= 1;
	...
	codec1->dai              = &soc_codec_cx2074x_cx20865_dai;
	codec1->num_dai          = 1;
	...
	soc_codec_cx2074x_cx20865_dai.dev = codec->dev;

	cx2074x_codec = codec;
	cx20865_codec = codec1;

	ret = snd_soc_register_codec(cx20865_codec);
	if (ret != 0) {
	...
	ret = snd_soc_register_dais(&soc_codec_cx2074x_cx20865_dai, 1);
	if (ret != 0) {
...
static void cx2074x_unregister(struct cx2074x_priv *cx2074x)
{
	...
	snd_soc_unregister_dai(&soc_codec_cx2074x_cx20865_dai);
...
static int cx20865_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	struct	snd_soc_codec *codec;
	Command cmd;
...
void CodecPowerOnOff(int On)
{
	...
	if (On) {
		...
		if (cx2074x->pwr_state == 3) {
			/* first turn on CX20865 & 20745 */
			cx2074x->gpio->GPIOxOUT |= GPIO_CODEC_POWER_ON;
			// GPIO setting to turn on
			msleep(500); // wait 500ms is enough?
			/* Next wait a 500ms, send command to CX20865 to turn on 24.576MHz clock */
			/* golem::sendcmd CTRL CONTROL_APP_SET_MEMORY {MEM_TYPE_Y, 0x3F000, 0x12, 0xFFFFFFFF} */
...
static int __init cx2074x_modinit(void)
{
	...
	if (ret != 0)
		ERROR("Failed to register CX207865 I2C driver: %d\n", ret);
