...
#define NX_PCM_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FMTBIT_S32_LE)
...
static int alloc_dma_ch(struct snd_pcm_substream *substream)
{
	...
	DBGOUT("%s (ch:%d) spdif=%d, card=%d, device=%d, stream=%d%s\n", __func__, ch, spdif, card, device, stream, substream->pcm->card->driver);
	if (-1 == ch)
		return ret;
...
static int start_dma_ch(struct snd_pcm_substream *substream)
{
	...
	DBGOUT("%s : card=%d, device=%d, stream=%d, length=%d\n", __func__, card, device, stream, snd_pcm_lib_period_bytes(substream));
	if (! tr)
		return -1;
