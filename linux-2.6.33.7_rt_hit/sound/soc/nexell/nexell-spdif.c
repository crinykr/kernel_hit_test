...
{
    struct	NX_SPDIF_RegisterSet *spdif_reg = nx_spdif.regs;
    int clksel1;
...
static irqreturn_t nx_spdif_irq(int irq, void *dev_id)
{
    ...
    pend = nx_spdif->regs->SPDIF_PEND;

    if ((pend & NXP2120_SPDIF_PEND_LOCK)) { /* Interrupt pending bit of LOCK detect. */
    	nx_spdif->regs->SPDIF_PEND = nx_spdif->regs->SPDIF_PEND | NXP2120_SPDIF_PEND_LOCK;

	 	nx_spdif->regs->SPDIF_CONTROL = nx_spdif->regs->SPDIF_CONTROL | NXP2120_SPDIF_CTRL_EN_DECODE;
    	DBGOUT(KERN_ERR "Interrupt pending bit of LOCK detect.\n");
    }
    //if (nx_spdif.debug_mask)
    {
	    if ((pend & NXP2120_SPDIF_PEND_ERR)) { /* Interrupt pending bit of ERROR detect. */
	    	nx_spdif->regs->SPDIF_PEND = nx_spdif->regs->SPDIF_PEND | NXP2120_SPDIF_PEND_ERR;
	        DBGOUT(KERN_ERR "Interrupt pending bit of ERROR detect.\n");
	    }
	    if ((pend & NXP2120_SPDIF_PEND_PARITY)) { /* Interrupt pending bit of PARITY detect */
	    	nx_spdif->regs->SPDIF_PEND = nx_spdif->regs->SPDIF_PEND | NXP2120_SPDIF_PEND_PARITY;
	    	DBGOUT(KERN_ERR "Interrupt pending bit of PARITY detect\n");
	    }

	    if ((pend & NXP2120_SPDIF_PEND_BLOCK)) { /* Interrupt pending bit of PARITY detect */
	    	nx_spdif->regs->SPDIF_PEND = nx_spdif->regs->SPDIF_PEND | NXP2120_SPDIF_PEND_BLOCK;
	    	DBGOUT(KERN_ERR "Interrupt pending bit of BLOCK detect\n");
	    }
	    return IRQ_HANDLED;
	}
                  ...
                  struct snd_soc_dai *dai)
{
	...
	nx_spdif.regs->SPDIF_PEND = 0xF0 | nx_spdif.debug_mask;
...
static int start_nx_spdif(struct snd_pcm_substream *substream)
{
	...
	spdif_reg->SPDIF_PEND    = 0xF0 | NXP2120_SPDIF_PEND_EN_IRQ_LOCK;
	spdif_reg->SPDIF_CONTROL = spdif_reg->SPDIF_CONTROL | NXP2120_SPDIF_CTRL_EN_PHA_DET;
...
static void stop_nx_spdif(struct snd_pcm_substream *substream)
{
	...
	spdif_reg->SPDIF_CONTROL = spdif_reg->SPDIF_CONTROL & ~(NXP2120_SPDIF_CTRL_EN_DECODE | NXP2120_SPDIF_CTRL_EN_PHA_DET);
	spdif_reg->SPDIF_PEND    = 0xF0;//spdif_reg->SPDIF_PEND & ~(NXP2120_SPDIF_PEND_EN_IRQ_ERR | NXP2120_SPDIF_PEND_EN_IRQ_PARITY | NXP2120_SPDIF_PEND_EN_IRQ_BLOCK);
	// Do something
}
...
#define NXP2120_SPDIF_RATES (SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_44100 | SNDRV_PCM_RATE_16000)

#define NX_SPDIF_FORMATS (SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FMTBIT_S16_LE)
...
struct snd_soc_dai nxp2120_spdif_dai = {
    ...
    .capture = {
        ...
        .formats = NX_SPDIF_FORMATS,}, // 24bit only?...
