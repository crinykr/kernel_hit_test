...
#define READ8(cl, reg, val) \
	do { \
		err = mt9m114_reg_read8(cl, reg, val); \
		if (err != 0) \
			goto err; \
	} while (0)

#define WRITE8(cl, reg, val) \
	do { \
		err = mt9m114_reg_write8(cl, reg, val); \
		if (err != 0) \
			goto err; \
	} while (0)

#define READ16(cl, reg, val) \
	do { \
		err = mt9m114_reg_read16(cl, reg, val); \
		if (err != 0) \
			goto err; \
	} while (0)

#define WRITE16(cl, reg, val) \
	do { \
		err = mt9m114_reg_write16(cl, reg, val); \
		if (err != 0) \
			goto err; \
	} while (0)

#define WRITE32(cl, reg, val) \
	do { \
		err = mt9m114_reg_write32(cl, reg, val); \
		if (err != 0) \
			goto err; \
	} while (0)

#define POLL16(cl, reg, mask, val) \
	do { \
		err = mt9m114_reg_poll16(cl, reg, mask, val, 10, 100); \
		if (err != 0) \
			goto err; \
	} while (0)

#define RMW16(cl, reg, set, unset) \
	do { \
		err = mt9m114_reg_rmw16(cl, reg, set, unset); \
		if (err != 0) \
			goto err; \
	} while (0)

#define WRITEARRAY(cl, array) \
	do { \
		err = mt9m114_reg_write_array(cl, array, \
					      ARRAY_SIZE(array)); \
		if (err != 0) \
			goto err; \
	} while (0)
...
static const struct mt9m114_reg mt9m114_defaults[] = {
	...
	{ MT9M114_CAM_SYSCTL_PLL_ENABLE,		0x00, REG_U8 }, // PLL enable for 1 else 0
	{ MT9M114_CAM_SYSCTL_PLL_DIVIDER_M_N,		0x0120 },
	{ MT9M114_CAM_SYSCTL_PLL_DIVIDER_P,		0x0700 },
	{ MT9M114_CAM_PORT_OUTPUT_CONTROL,		0x8040 }, // 8040 for Parallel, 8041 for MIPI
	//{ MT9M114_CAM_OUTPUT_FORMAT_YUV,        0x0019 }, // 0 even UV, 1 odd UV, 2 evenUoddV
	//{ MT9M114_CAM_OUTPUT_FORMAT,            0x0010 }, // make mono output 0x14, else 0x10
	...
	/* Enable LED */
	{ MT9M114_PAD_CONTROL,			0x0fD9 }, // 0xFD8 is default
			     ...
			     u16 set, u16 unset)
{
	...
	READ16(client, reg, &val);

	val |= set;
	val &= ~unset;

	WRITE16(client, reg, val);
...
static int mt9m114_reg_write_array(struct i2c_client *client, const struct mt9m114_reg *regarray, int regarraylen)
{
	int i;
	int err;
	...
	for (i = 0; i < regarraylen; i++) {
		...
		case REG_U16:
			WRITE16(client, regarray[i].reg, (u16)regarray[i].val);
			break;
		case REG_U8:
			WRITE8(client, regarray[i].reg, (u8)regarray[i].val);
			break;

		case REG_U32:
			WRITE32(client, regarray[i].reg, (u32)regarray[i].val);
			break;
		default:
			BUG_ON(1);
		}
...
static int mt9m114_get_patch_id(struct i2c_client *client, u16 *patch_id)
{
	...
	READ8(client, MT9M114_PATCHLDR_NUM_PATCHES, &patch_index);
	...
	READ16(client, MT9M114_PATCHLDR_PATCH_ID_0 + 2*(patch_index-1), patch_id);
...
static int mt9m114_apply_firmware_patch(struct i2c_client *client)
{
	...
	WRITE16(client, MT9M114_ACCESS_CTL_STAT, 0x0001);
	WRITE16(client, MT9M114_PHYSICAL_ADDRESS_ACCESS, 0x5000);

	WRITEARRAY(client, mt9m114_firmware_patch);

	WRITE16(client, MT9M114_LOGICAL_ADDRESS_ACCESS, 0x0000);
	WRITE16(client, MT9M114_PATCHLDR_LOADER_ADDRESS, 0x010c);
	WRITE16(client, MT9M114_PATCHLDR_PATCH_ID, 0x0202);
	WRITE32(client, MT9M114_PATCHLDR_FIRMWARE_ID, 0x41030202);
	WRITE16(client, MT9M114_COMMAND_REGISTER, 0xfff0);

	POLL16(client, MT9M114_COMMAND_REGISTER, MT9M114_COMMAND_REGISTER_APPLY_PATCH,
	       0x0000);

	WRITE16(client, MT9M114_COMMAND_REGISTER, 0xfff1);

	POLL16(client, MT9M114_COMMAND_REGISTER, MT9M114_COMMAND_REGISTER_APPLY_PATCH,
	       0x0000);

	READ8(client, MT9M114_PATCHLDR_APPLY_STATUS, &status);
	if (status != 0x00) {
		err = -EFAULT;
		goto err;
	}
			     ...
			     u8 next_state, u8 final_state)
{
	...
	WRITE16(client, MT9M114_LOGICAL_ADDRESS_ACCESS, MT9M114_SYSMGR_NEXT_STATE);
	WRITE8(client, MT9M114_SYSMGR_NEXT_STATE, next_state);
	
	/* Make sure FW is ready to set state. */
	POLL16(client, MT9M114_COMMAND_REGISTER,
	       MT9M114_COMMAND_REGISTER_SET_STATE,
	       0x0000);
	
	/* Set master sreaming last */
	WRITE16(client, MT9M114_COMMAND_REGISTER, (MT9M114_COMMAND_REGISTER_OK |
					   MT9M114_COMMAND_REGISTER_SET_STATE));
	...
	READ8(client, MT9M114_SYSMGR_CMD_STATUS, &status);
	...
	READ8(client, MT9M114_SYSMGR_CURRENT_STATE, &state);
...
static int mt9m114_change_config(struct i2c_client *client)
{
	...
	READ8(client, MT9M114_SYSMGR_CURRENT_STATE, &old_state);

	/* Set state to change-config. */
	WRITE8(client, MT9M114_SYSMGR_NEXT_STATE,
	...
	READ16(client, MT9M114_COMMAND_REGISTER, &cmd);
...
static int mt9m114_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	WRITE16(client, MT9M114_LOGICAL_ADDRESS_ACCESS, 0x4834);

	if (cam->flip_flag & NX_VIP_FLIP_FLAG_VFLIP) {
		RMW16(client, MT9M114_CAM_SENSOR_CONTROL_READ_MODE, 0x0002, 0x0000);
	} else {
		RMW16(client, MT9M114_CAM_SENSOR_CONTROL_READ_MODE, 0x0000, 0x0002);
	}

	if (cam->flip_flag & NX_VIP_FLIP_FLAG_HFLIP) {
		RMW16(client, MT9M114_CAM_SENSOR_CONTROL_READ_MODE, 0x0001, 0x0000);
	} else {
		RMW16(client, MT9M114_CAM_SENSOR_CONTROL_READ_MODE, 0x0000, 0x0001);
	}
...
static int mt9m114_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
	case V4L2_CID_VFLIP:
		ctrl->value = (cam->flip_flag & NX_VIP_FLIP_FLAG_VFLIP) != 0;
		break;
	case V4L2_CID_HFLIP:
		ctrl->value = (cam->flip_flag & NX_VIP_FLIP_FLAG_HFLIP) != 0;
		break;
	default:
		return -EINVAL;
	}
...
static int mt9m114_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
	case V4L2_CID_VFLIP:
		cam->flip_flag |= ctrl->value ? NX_VIP_FLIP_FLAG_VFLIP : 0;
		break;
	case V4L2_CID_HFLIP:
		cam->flip_flag |= ctrl->value ? NX_VIP_FLIP_FLAG_HFLIP : 0;
		break;
	default:
		return -EINVAL;
	}
...
//	enum v4l2_mbus_pixelcode code = mf->code;
	...
	mt9m114_res_roundup(&fmt->fmt.pix.width, &fmt->fmt.pix.height);
...
*/	
...
#endif
	//mt9m114_s_stream(sd, false); // ksw : before change stop streming

	//WRITE16(client, MT9M114_CHAIN_CONTROL, 0xF100); /* Sync enable and master */
	//WRITE16(client, MT9M114_CHAIN_CONTROL, 0xD000); /* Sync enable and slave */

	WRITEARRAY(client, mt9m114_defaults);
	WRITE16(client, MT9M114_DEVICE_ADDR_ID, 0xBABA); /* SADDR 0/1 as same address 0xBA */ 
	...
	err = mt9m114_set_res(client, fmt->fmt.pix.width, fmt->fmt.pix.height);
	if (err < 0)
		goto err;
...
static int mt9m114_video_probe(struct i2c_client *client) //struct soc_camera_device *icd,
{
	 ...
	 */
	 READ16(client, MT9M114_CHIP_ID, &chip_id);
	 READ16(client, MT9M114_CUSTOMER_REV, &chip_revision);
...
static int mt9m114_command(struct i2c_client *client, u32 cmd, void *arg)
{
	switch (cmd) {
	case I2C_CAM_INIT:
		dev_info(&client->dev, "MT9M114 : external camera initialized\n");
		break;
	...
	default:
		dev_err(&client->dev, "unexpect command\n");
		break;
	}
			 ...
			 const struct i2c_device_id *did)
{
	...
	if (strstr(did->name, "_2"))
		info->id = 1;
	else
		info->id = 0;

	printk("MT9M114 name = %s, id=%d\n", did->name, info->id);
	
	info->client = client;
//	info->i2c_read = mt9m114_get_i2c;
//	info->i2c_write = mt9m114_set_i2c;
...
#if defined(CONFIG_CAMERA1_MT9M114)
...
#if defined(CONFIG_CAMERA2_MT9M114)
static const struct i2c_device_id mt9m114_id2[] = {
	{ "mt9m114_2", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, mt9m114_id2);

static struct i2c_driver mt9m114_driver2 = {
	.driver = {
		.name = "MT9M114_2",
	},
	.probe    = mt9m114_probe,
	.remove   = mt9m114_remove,
	.id_table = mt9m114_id2,
};
#endif
...
#endif
#if defined(CONFIG_CAMERA2_MT9M114)
	ret = i2c_add_driver(&mt9m114_driver2);
#endif
	return ret;
}
...
#endif
#if defined(CONFIG_CAMERA2_MT9M114)
	i2c_del_driver(&mt9m114_driver2);
#endif
}
