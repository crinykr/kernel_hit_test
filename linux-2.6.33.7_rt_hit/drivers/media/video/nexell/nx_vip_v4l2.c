					...
					struct v4l2_format *f)
{
	...
	if (f->fmt.pix.priv == V4L2_FMT_IN)
		...
		if ((f->fmt.pix.width != ctrl->in_cam->cur_width) ||
			(f->fmt.pix.height != ctrl->in_cam->cur_height)) {
			...
			fmt.fmt.win.w.width     = ww;
			fmt.fmt.win.w.height    = hh;
			fmt.fmt.pix.width       = ww;
			fmt.fmt.pix.height      = hh;
			fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV;
			fmt.fmt.pix.field       = V4L2_FIELD_SEQ_TB;
			
			/* Now camera can handle this resolution */
			ret = v4l2_subdev_call(&cam->sd, video, try_fmt, &fmt);
			if (0 == ret) {
				...
				struct v4l2_buffer *b)
{
	...
	index = ctrl->buf_index - 1;
	if (index < 0)
		index = frame->nr_frames-1;
	b->index =  index;
	b->m.offset = frame->addr[index].phys_y;
	frame->skip_frames[index] = 1;
					...
					struct v4l2_cropcap *a)
{
	...
	ctrl->v4l2.crop_bounds.width = ctrl->in_cam->max_width;
	ctrl->v4l2.crop_bounds.height = ctrl->in_cam->max_height;
	...
	ctrl->v4l2.crop_defrect.width = ctrl->in_cam->def_width;
	ctrl->v4l2.crop_defrect.height = ctrl->in_cam->def_height;
